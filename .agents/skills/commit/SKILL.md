...
2. `git branch --show-current` で現在ブランチを確認し、`feature/issue-<num>-...` 形式なら Issue番号を自動抽出する
3. `git diff --staged` を読み、変更点を要約する（必要なら `git diff` も）
4. 変更ファイルの一覧と、UI影響がありそうな箇所を抽出する
5. 出力は.md形式で以下の順で行う:
...
- 不要なセクションは削る
- Issue番号の自動抽出ルール:
  - 現在ブランチが `feature/issue-<num>-...` に一致する場合は `<num>` を関連Issueとして使う
