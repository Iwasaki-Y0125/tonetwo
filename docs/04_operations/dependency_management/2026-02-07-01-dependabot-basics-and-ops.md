...
## GitHub Actions更新メモ（@v6 と @SHA）
### `uses: actions/checkout@v6` とは
- CI実行マシンにリポジトリのコードを取得する処理。
- `@v6` は checkout action のバージョンタグ参照。
...
- `@SHA`:
  - 特定コミット固定で再現性が高い。
  - 自動追従しないため更新管理コストが上がる。

### サプライチェーン攻撃
- タグ参照（`@v6`）は便利だが、`v6` 系の更新を自動で取り込むため、供給元で問題のあるパッチが公開された場合に、自動で反映されてサプライチェーン攻撃の影響を受ける可能性がある。
- SHA固定（`@SHA`）は特定コミットに固定されるため、供給元で新しい更新が出ても勝手に切り替わらず、同種のリスクを抑えやすい。

### このリポジトリの当面方針
- 現段階は `@v6` 運用を許容する。
- 高リスク機能の場合は `@SHA` 固定運用へ段階移行を検討する。

### 会社で `@SHA` 管理が強く求められやすい案件
- 金融/決済/医療/行政など高リスク領域
- 大企業向けB2Bでセキュリティ監査が入る案件
- SOC2/ISO27001 などの準拠が必要な案件
- 供給網リスク対策を契約要件で求められる案件
